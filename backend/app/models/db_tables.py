...
class AgentTrajectoryTable(Base):
    __tablename__ = "agent_trajectory"
    ...
    token_usage: Mapped[dict | None] = mapped_column(JSONB, default=None)
    prompt_versions: Mapped[dict | None] = mapped_column(JSONB, default=None)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
