...
- 允许用户在未上传待审合同时直接发送审查请求，并返回明确的空结果提示。
...
7. 决策：合同审查模式下不在前端强制要求先上传待审合同，而是在后端返回显式空结果。  
   - Rationale: 用户仍然可以先切换模式、输入问题，再根据返回结果决定是否补传合同，交互更顺滑。  
   - Alternatives considered:
     - 前端发送前阻止：校验简单，但会增加一次额外操作，并与最新交互预期冲突。
     - 后端静默成功但不给提示：用户无法判断是没有合同还是审查本身失败。
...
- [Risk] 未上传合同时仍允许发送可能让用户误以为系统已开始正式审查 -> Mitigation: 后端返回固定、明确的“当前无合同可审查”提示，不生成伪审查内容。
